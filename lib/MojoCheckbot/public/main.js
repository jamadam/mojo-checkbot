$(function() {
    ...
    function fetch() {
        ...
        $.get('/diff?offset='+offset, function(msg){
            $.each(msg.result, function(idx, data){
                ...
                newTr.data('method', data[dataKey.method]);
    ...
    $("tbody tr").live('click', function() {
        ...
        else if ($(this).data('dialog') && $(this).data('dialog').names) {
            lb = $('#trLightboxForm');
            var names = $(this).data('dialog').names;
            var cont = $('#commonForm table');
            cont.empty();
            for (var idx in names) {
                var input = $('<input />', {type:'text', name:names[idx]});
                var th = $('<th/>').html(names[idx]);
    ...
    $("#commonForm").bind('submit', function(){
        $.post('/post', {
            'url'       : $("#trLightboxForm .resolvedURI textarea").html(),
            'method'    : $('#trLightboxForm').data('originalTr').data('method'),
